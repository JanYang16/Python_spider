...
def get_city_urls():
    ...
    try:
        ...
        soup = BeautifulSoup(response, 'lxml')                   # 解析response
        urls = soup.select('#destination_nav > div > div > div > dl.dl-list > dt > a')  # 提取城市标签列表
...
url_list_url = set([i['url'] for i in url_list.find()])           # 获取url_list表里的除重url
item_info_url = set([i['url'] for i in item_info.find()])         # 获取item_info表里的除重url
req_url = url_list_url - item_info_url                            # 得到未爬取详情页的url
...
if __name__ == '__main__':
    ...
    pool.map(get_item_info, req_url)                              # 多进程爬取详情页数据（断点续爬）
    pool.close()                                                  # 等待进程池中的进程执行结束后再关闭pool
    pool.join()                                                   # 防止主进程在子进程结束前提前关闭
    # get_item_info('http://www.senseluxury.com/villa/2539_levelfurnishedlivingapartment3bedroom')
